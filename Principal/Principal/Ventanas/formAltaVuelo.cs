            ...
            if (Validaciones() == false)
            {
                if (ValidacionAeropuerto() == false)
                {
                    ...
                    if (resultado == System.Windows.Forms.DialogResult.Yes)
                    {
                        try
                        {
                            var fechaSalida = calendarioSalida.SelectionRange.Start.ToShortDateString();
                            var fechaLlegada = calendarioLlegada.SelectionRange.Start.ToShortDateString();

                            var horaSalida = cmbHoraSalida.SelectedItem.ToString();
                            var horaLlegada = cmbHoraLlegada.SelectedItem.ToString();

                            var fechaHoraS = fechaSalida + ' ' + horaSalida;
                            var fechaHoraL = fechaLlegada + ' ' + horaLlegada;

                            var fechaHoraSalida = Convert.ToDateTime(fechaHoraS);
                            var fechaHoraLlegada = Convert.ToDateTime(fechaHoraL);
